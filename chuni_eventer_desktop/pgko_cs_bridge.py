...
def _candidate_bridge_paths() -> list[Path]:
    ...
    out.extend(
        [
            ...
            (root / "tools" / "PenguinBridge" / "bin" / "Release" / "net8.0" / "win-x64" / "publish" / "PenguinBridge.exe").resolve(),
...
def _candidate_core_dll_paths() -> list[Path]:
    env = os.environ.get("CHUNI_PENGUIN_TOOLS_CORE_DLL", "").strip()
    out: list[Path] = []
    if env:
        out.append(Path(env).expanduser().resolve())
    root = app_root_dir()
    ...
    out.extend(
        [
            (root / "PenguinTools" / "PenguinTools.Core" / "bin" / "Release" / "net8.0" / "PenguinTools.Core.dll").resolve(),
            (root / "PenguinTools" / "PenguinTools.Core" / "bin" / "Release" / "net10.0" / "PenguinTools.Core.dll").resolve(),
            (root / "PenguinTools" / "PenguinTools.Core" / "bin" / "Release" / "net9.0" / "PenguinTools.Core.dll").resolve(),
            (root / "PenguinTools.Core.dll").resolve(),
            (cwd / "PenguinTools.Core.dll").resolve(),
        ]
    )
    unique: list[Path] = []
    seen: set[str] = set()
...
def resolve_penguin_tools_core_dll() -> Path | None:
    for p in _candidate_core_dll_paths():
        if p.exists() and p.is_file():
            return p
    return None
...
def convert_mgxc_with_penguin_bridge(*, input_mgxc: Path, output_c2s: Path) -> None:
    ...
    env = os.environ.copy()
    # 自动补齐 Core 路径，避免每次手动设置环境变量。
    if not env.get("CHUNI_PENGUIN_TOOLS_CORE_DLL"):
        core = resolve_penguin_tools_core_dll()
        if core is not None:
            env["CHUNI_PENGUIN_TOOLS_CORE_DLL"] = str(core)
    p = subprocess.run(
        cmd,
        capture_output=True,
        text=True,
