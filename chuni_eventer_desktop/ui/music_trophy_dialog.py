...
from qfluentwidgets import BodyLabel, ComboBox as FluentComboBox, PrimaryPushButton, PushButton
...
class MusicTrophyDialog(FluentCaptionDialog):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._hint.setTextColor("#6B7280", "#9CA3AF")

        self._reload_combo(preselect)

        btn_expert = PushButton("生成 Expert 金 (rare 4)", self)
        btn_expert.clicked.connect(lambda: self._gen_one("expert"))
        btn_master = PushButton("生成 Master 铂 (rare 6)", self)
        btn_master.clicked.connect(lambda: self._gen_one("master"))
        self._btn_ultima = PushButton("生成 Ultima 课题 (rare 8)", self)
        self._btn_ultima.clicked.connect(lambda: self._gen_one("ultima"))
        btn_all = PrimaryPushButton("一键生成（本曲全部适用）", self)
    ...
    def _current_music(self) -> MusicItem | None:
        ...
        return d if isinstance(d, MusicItem) else None
    ...
    def _reload_combo(self, preselect: MusicItem | None) -> None:
        ...
        for i, it in enumerate(items):
            ...
            self._combo.addItem(f"{it.name.id} — {it.name.str}{suffix}", None, it)
    ...
    def _gen_one(self, kind: str) -> None:
        m = self._current_music()
        if m is None:
            fly_warning(self, "提示", "没有可选择的乐曲。")
            return
        if kind == "ultima" and not m.has_ultima:
            fly_warning(self, "提示", "当前乐曲没有已启用的 Ultima 难度，无法生成 Ultima 课题称号。")
            return
        try:
